...
public class BookListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.title)).setText("موضوع : " + getIntent().getStringExtra("name"));
        recyclerView = findViewById(R.id.recycler);
        adapters = new BookAdapters(context, books);
        recyclerView.setLayoutManager(new LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false));
        recyclerView.setAdapter(adapters);

        new JsonTask().execute("http://192.168.43.120/server/public/api/books?category=" + getIntent().getStringExtra("id"));
    }
